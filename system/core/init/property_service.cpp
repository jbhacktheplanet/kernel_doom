...
//[ZUIR-4220]Create property after reset factory.  by zhulei7 11/12/2020 start
static void set_init_adbwipe() {
    int fdwipe = -1;
    int times = 5;
    bool findFile = false;
    char flag[2] = {0};
    int length = 0;
    LOG(ERROR) << "[EASYIMAGE] set_init_adbwipe" << "\n";

    while(times--) {
        fdwipe = open("/mnt/vendor/persist/flag/adbwipe.txt", O_RDONLY);
        if(fdwipe >= 0) {
            findFile = true;
            break;
        } else {
            LOG(ERROR) << "[EASYIMAGE] open /mnt/vendor/persist/flag/adbwipe.txt failed! "
                       << strerror(errno);
        }
    }

    if(findFile != true) {
        LOG(ERROR) << "[EASYIMAGE] file[/mnt/vendor/persist/flag/adbwipe.txt] not exist!! \n";
    } else {
        length = read(fdwipe, flag, 1);
        //property_set("ro.lenovo.adbwipe", "true");
        close(fdwipe);

        if(length <= 0) {
            LOG(ERROR) << "[EASYIMAGE] fail to read /mnt/vendor/persist/flag/adbwipe.txt! "
                       << strerror(errno);
            return;
        }
        if((flag[0] != 0) && (strncasecmp(flag, "t", 1) == 0)) {//after reset factory first start device
            InitPropertySet("persist.sys.lenovo.adb_wipe", "0");  //change txt to "f"
            InitPropertySet("persist.sys.lenovo.wipe", "true");
        }

    }

}
//[ZUIR-4220]Create property after reset factory.  by zhulei7 11/12/2020 end
    ...
    if(CCode[0] != 0) {
        for(count = 0; count < ITEMS; count = count + 3) {
            if(strncasecmp(CCode, LocaleSet[count], 2) == 0) {
                ...
                if(sel == 0) {
                    InitPropertySet("ro.product.locale", tmp);
                    InitPropertySet("ro.product.locale.language", LocaleSet[count + 1]);
                    InitPropertySet("ro.product.locale.region", LocaleSet[count + 2]);
                    LOG(ERROR) << "[EASYIMAGE] set ro properties done. \n";
                } else {
                    InitPropertySet("persist.sys.language", LocaleSet[count + 1]);
                    InitPropertySet("persist.sys.country", LocaleSet[count + 2]);
                    InitPropertySet("persist.log.tag.country.code", CCode);
                    InitPropertySet("persist.sys.withsim.country", CCode);
                    InitPropertySet("ro.oem.key1", CCode); // Add by wangwq14, ZUIQ-13250, PAI feature.
