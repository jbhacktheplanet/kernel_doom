...
void adbd_auth_confirm_key(atransport* t) {
    bool value = android::base::GetBoolProperty("persist.sys.zui.dis_adb_auth", false);
    bool adb_debug_on = android::base::GetBoolProperty("ro.boot.adbon", false);
    LOG(INFO) << "prompting user to authorize key";
    t->AddDisconnect(&adb_disconnect);
    LOG(INFO) << "prompting user to authorize key"<< "value="<<value <<"adb_debug_on="<< adb_debug_on;
    if (value||adb_debug_on) {
        if(t){
    LOG(INFO) << "adbd_auth_verified";
