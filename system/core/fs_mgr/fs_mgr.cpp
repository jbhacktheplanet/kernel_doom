...
static void resize_f2fs(const std::string& blk_device)
{
    ...
    if (access(F2FS_RESIZE_BIN, X_OK)) {
        LERROR << "Not running " << F2FS_RESIZE_BIN << " on " << blk_device << " (executable not in system image)";
    } else {
        ret = logwrap_fork_execvp(ARRAY_SIZE(resizf2fs_argv), resizf2fs_argv, &status,
						    true, LOG_KLOG | LOG_FILE, true, FSCK_LOG_FILE);
        if (ret < 0) {
