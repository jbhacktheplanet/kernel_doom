		...
		if (!aw8697x_haptic_rtp_get_fifo_afi(aw8697x)) {
			...
			if (aw8697x->rtp_cnt != aw8697x_rtp->len) {
				if (aw8697x->timeval_flags == 1) {
					//do_gettimeofday64(&aw8697x->start);
			    ...
			    const struct i2c_device_id *id)
{
	...
	pr_info("%s aw8697x->reset_gpio = %d, aw8697x->irq_gpio=%d\n", __func__,
		aw8697x->reset_gpio, aw8697x->irq_gpio);
...
static int __init aw8697x_i2c_init(void)
{
	...
	pr_info("aw8697x driver version %s\n", AW8697X_VERSION);
