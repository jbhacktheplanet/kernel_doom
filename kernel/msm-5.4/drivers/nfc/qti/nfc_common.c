...
struct clk *nq_clk = NULL;
bool clk_run = false;
bool clk_manual = false;
 ...
 */
static int nqx_clock_select(struct nfc_dev *dev)
{
	int r = 0;

	pr_info("[nfc]clk select entry\n");
	nq_clk = clk_get(dev->nfc_device, "ref_clk");

	if (nq_clk == NULL) {
		pr_err("[nfc]clk_get failed!!!!!\n");
		goto err_clk;
	}

	if (!clk_run)
		r = clk_prepare_enable(nq_clk);

	if (r) {
		pr_err("[nfc]clk prepare_enable failed!!!!!\n");
		goto err_clk;
	}
 ...
 */
static int nqx_clock_deselect(struct nfc_dev *dev)
{
	int r = -1;

	pr_info("[nfc]clk deselect entry\n");
	if (nq_clk != NULL) {
		if (clk_run) {
			clk_disable_unprepare(nq_clk);
			clk_run = false;
		}
		return 0;
	}
...
int nfc_ese_pwr(struct nfc_dev *nfc_dev, unsigned long arg)
{
	...
	if (arg == ESE_POWER_ON) {
		...
		if (!nfc_dev->nfc_ven_enabled) {
			pr_info("eSE HAL service setting ven HIGH\n");
			gpio_set_ven(nfc_dev, 1);
		} else {
			pr_info("ven already HIGH\n");
		}
		nfc_dev->is_ese_session_active = true;
	} else if (arg == ESE_POWER_OFF) {
		if (!nfc_dev->nfc_ven_enabled) {
			pr_warn("NFC not enabled, but remain ven enabled!!\n");
			//gpio_set_ven(nfc_dev, 0);
		} else {
			pr_info("keep ven high as NFC is enabled\n");
		}
...
static int nfc_ioctl_power_states(struct nfc_dev *nfc_dev, unsigned long arg)
{
	...
	if (arg == NFC_POWER_OFF) {
		...
		if (gpio_is_valid(nfc_dev->gpio.dwl_req)) {
			...
			pr_info("gpio.dwl_req is invalid\n");
...
err_nfcc_hw_check:
	...
	ret = -ENXIO;
	pr_info("%s: - NFCC HW not available\n", __func__);
