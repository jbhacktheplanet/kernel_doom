...
static int dwc3_msm_usb_set_role(struct device *dev, enum usb_role role)
{
...
#endif
		gpio_set_value(mdwc->charge_1t1_gpio, 1);
		gpio_set_value(mdwc->charge_1t2_gpio, 1);
		msleep(10);
		gpio_set_value(mdwc->otg_gpio, 1);
		break;
	...
	case USB_ROLE_NONE:
		...
		mdwc->id_state = DWC3_ID_FLOAT;
		if(usb1_otg_en == 1){
		pr_info("dwc3 usb1_otg_en = %d\n", usb1_otg_en );
		usb1_otg_en = 0;
#ifdef CONFIG_QGKI
		connect_device_type = 0;
#endif
		gpio_set_value(mdwc->otg_gpio, 0);
		gpio_set_value(mdwc->charge_1t1_gpio, 0);
		gpio_set_value(mdwc->charge_1t2_gpio, 0);
		}
		break;
	}
...
static int dwc3_otg_pinctrl_init(struct dwc3_msm *chg, struct platform_device *pdev)
{
	int retval;

	/* Get pinctrl if target uses pinctrl */
	chg->otg_pinctrl = devm_pinctrl_get(&pdev->dev);
...
static void dwc3_init_otg_gpio(struct dwc3_msm *chg, struct platform_device *pdev)
{
	int ret;

	ret = dwc3_otg_pinctrl_init(chg, pdev);
	...
	chg->otg_gpio = of_get_named_gpio(pdev->dev.of_node,
					"qcom,usb1_otg_en", 0);
	gpio_otg1_status = chg->otg_gpio;
	...
	chg->charge_1t1_gpio = of_get_named_gpio(pdev->dev.of_node,"qcom,usb_charge_1t1_en",0);
       if(!gpio_is_valid(chg->charge_1t1_gpio)){
		 pr_err("DWC3  %s - Error: gpio pin invalid! Pin value: %d\n", __func__, chg->charge_1t1_gpio);
		 return;
	}
       pr_info("DWC3 - %s, Pin value: %d\n", __func__, chg->charge_1t1_gpio);

	chg->charge_1t2_gpio = of_get_named_gpio(pdev->dev.of_node,"qcom,usb_charge_1t2_en",0);
       if(!gpio_is_valid(chg->charge_1t2_gpio)){
		 pr_err("DWC3  %s - Error: gpio pin invalid! Pin value: %d\n", __func__, chg->charge_1t2_gpio);
		 return;
	}
       pr_info("DWC3 - %s, Pin value: %d\n", __func__, chg->charge_1t2_gpio);
	}
