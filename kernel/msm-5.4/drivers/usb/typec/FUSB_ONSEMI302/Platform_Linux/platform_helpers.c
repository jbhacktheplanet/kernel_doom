...
#include "../../../../power/supply/sc/pd_policy_manager.h"
bool fusb_orient_en;
int gpio_otg2_status;
#endif
...
FSC_S32 fusb_InitializeGPIO(void)
{
    ...
    gpio_otg2_status = chip->gpio_OTG;
    ...
    gpio_direction_output(chip->gpio_OTG, 0);
...
static const struct typec_operations fusb_ops = {
	.dr_set = NULL,
	.pr_set = NULL
};
...
void fusb_InitializeCore(void)
{
    ...
    chip->typec_caps.fwnode = NULL;
    chip->typec_caps.driver_data = chip;
    chip->typec_caps.ops = &fusb_ops;
...
static irqreturn_t _fusb_isr_intn(FSC_S32 irq, void *dev_id)
{
    ...
    if (!chip->queued)
    {
      ...
      pr_debug("FUSB %s pm_stay_awake %d\n", __func__,__LINE__);
	...
	fusb_orient_en = true;
		...
		if (chip->port.sourceOrSink == SINK) {
			...
			//chip->typec_mode - POWER_SUPPLY_TYPEC_SOURCE_DEFAULT1);
	                  if (!chip->partner) {
			    memset(&chip->partner_desc, 0, sizeof(chip->partner_desc));
