...
{
       return qq_chip; // return a pointer to our structs^M
}
void fusb30x_charger_SetChip(struct charger_object* ccn_Chip)
{
	qq_chip = ccn_Chip;     // assign the pointer to our struct^M
}
#endif
...
#ifdef CONFIG_PRODUCT_DOOM
		charger->req_voltage = port->SrcCapsReceived[i].PPSAPDO.MaxVoltage * 5;
		charger->req_current = port->SrcCapsReceived[i].PPSAPDO.MaxCurrent;
...
#ifdef CONFIG_PRODUCT_DOOM
	pr_info("fusb302:typec_detach_handler\n");
	if (fusb_fetch_pdo_chip != NULL){
		fusb_fetch_pdo_chip->fusb302_pd_active = false;
		fusb_pd_enable = fusb_fetch_pdo_chip->fusb302_pd_active;
		}
