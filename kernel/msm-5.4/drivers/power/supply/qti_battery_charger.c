...
#ifdef CONFIG_LENOVO_SUPPLY
#define DEFAULT_USER_CAPACITY			15
#define DEFAULT_USER_TEMP			250
#define DEFAULT_USER_STATUS			0
#define DEFAULT_USER_HEALTH			0
#define DEFAULT_USER_VOLTAGE_NOW		0
#define DEFAULT_USER_CURRENT_NOW		0
#define DEFAULT_USER_CHARGE_COUNTER		0
#define DEFAULT_USER_CYCLE_COUNT		0
#define DEFAULT_USER_CHARGE_FULL_DESIGN		0
#define DEFAULT_USER_CHARGE_FULL		0
#endif
...
enum battery_property_id {
	...
	LB_FG1_CURR_NOW,
	LB_FG1_SOH,
	LB_FG1_TI_DIE_TEMPERATURE,
	LB_FG1_CHARGE_FULL,
	LB_FG1_FULL_DESIGN,
	LB_FG1_CHARGE_COUNTER,
	LB_FG1_CYCLE_COUNT,
	LB_FG1_TIME_TO_EMPTY_NOW,
	LB_FG1_TIME_TO_FULL_NOW,
	LB_FG1_CHIP_FW,
	HLOS_ADSP_CHG_ENABLE,
	HLOS_ADSP_CHG_PPS_ST,
	ADSP_FCC_VOTER_VALUE,
	ADSP_CP_THERMAL_CTR,
	BATT_SET_FV,
	LB_FG1_READ_ENABLE,
#endif
...
enum usb_property_id {
	...
	USB_TYPEC_COMPLIANT,
	USB_TYPEC_ORIENT,
	ADSP_USB_INPUT_SUSPEND_EN,
	HLOS_GET_CP_STEP,
	ADSP_ICL_SET_VALUE,
	FIXPDO1_MAX_CURRENT,
	FIXPDO2_MAX_CURRENT,
	FIXPDO2_MAX_POWER,
	HLOS_TYPEC_CONNECT_STATUS,
	USB_PROP_MAX,
};
...
#ifdef CONFIG_LENOVO_SUPPLY
	[LB_FG1_VOLT_NOW]	= LENOVO_SUPPLY_PROP_FG1_VOLTAGE_NOW,
	[LB_FG1_CURR_NOW]	= LENOVO_SUPPLY_PROP_FG1_CURRENT_NOW,
	[LB_FG1_SOH]		= LENOVO_SUPPLY_PROP_FG1_SOH,
	[LB_FG1_TI_DIE_TEMPERATURE] = LENOVO_SUPPLY_PROP_FG1_TI_DIE_TEMPERATURE,
	[LB_FG1_CHARGE_FULL]	= LENOVO_SUPPLY_PROP_FG1_CHARGE_FULL,
	[LB_FG1_FULL_DESIGN]	= LENOVO_SUPPLY_PROP_FG1_FULL_DESIGN,
	[LB_FG1_CHARGE_COUNTER]	= LENOVO_SUPPLY_PROP_FG1_CHARGE_COUNTER,
	[LB_FG1_CYCLE_COUNT]	= LENOVO_SUPPLY_PROP_FG1_CYCLE_COUNT,
	[LB_FG1_TIME_TO_EMPTY_NOW]  = LENOVO_SUPPLY_PROP_FG1_TIME_TO_EMPTY_NOW,
	[LB_FG1_TIME_TO_FULL_NOW]   = LENOVO_SUPPLY_PROP_FG1_TIME_TO_FULL_NOW,
	[LB_FG1_CHIP_FW]        = LENOVO_SUPPLY_PROP_FG1_CHIP_FW,
	[HLOS_ADSP_CHG_ENABLE]    =  LENOVO_SUPPLY_PROP_ADSP_CHG_ENABLE,
	[HLOS_ADSP_CHG_PPS_ST]	=	LENOVO_SUPPLY_PROP_ADSP_CHG_PPS_ST,
	[ADSP_FCC_VOTER_VALUE]	=	LENOVO_SUPPLY_PROP_ADSP_FCC_VALUE,
	[ADSP_CP_THERMAL_CTR]	=	LENOVO_SUPPLY_PROP_ADSP_CP_THERMAL_CTR,
	[BATT_SET_FV]				=	LENOVO_SUPPLY_PROP_SET_FV,
	[LB_FG1_READ_ENABLE]    =  LENOVO_SUPPLY_PROP_FG1_READ_ENABLE,
#endif
};
...
#ifdef CONFIG_LENOVO_SUPPLY
	[USB_TYPEC_ORIENT] = POWER_SUPPLY_PROP_TYPEC_ORIENT,
	[ADSP_USB_INPUT_SUSPEND_EN]	=	LENOVO_SUPPLY_PROP_ADSP_USB_SUSPEND_ENABLE,
	[HLOS_GET_CP_STEP]	=	LENOVO_SUPPLY_PROP_HLOS_GET_CP_STEP,
	[ADSP_ICL_SET_VALUE]	=	LENOVO_SUPPLY_PROP_ADSP_ICL_VALUE,
	[USB_REAL_TYPE]	=	LENOVO_SUPPLY_PROP_ADSP_REAL_TYPE,
#endif
	[FIXPDO1_MAX_CURRENT]	= 	LENOVO_SUPPLY_PROP_FIXPDO1_CURR,
	[FIXPDO2_MAX_CURRENT]	= 	LENOVO_SUPPLY_PROP_FIXPDO2_CURR,
	[FIXPDO2_MAX_POWER]	= 	LENOVO_SUPPLY_PROP_FIXPDO2_MAX_POWER,
	[HLOS_TYPEC_CONNECT_STATUS]	=	LENOVO_SUPPLY_PROP_TYPEC_CONNECT_ST,
};
				...
				enum power_supply_property prop, int *val)
{
	...
	default:
		...
		if (!rc)
			*val = pst->prop[prop_id];
		break;
	}

	return rc;
}
EXPORT_SYMBOL(qti_battery_charger_get_qcom_prop);

static int get_ext_property_id(struct psy_state *pst,
				...
				enum lenovo_supply_property prop, int *val)
{
	...
	if (prop >= LENOVO_SUPPLY_PROP_TOTLA_MAX)
		return -EINVAL;
	...
	case LENOVO_SUPPLY_PROP_FIXPDO2_MAX_POWER:
		pst = &bcdev->psy_list[PSY_TYPE_USB];
		prop_id = get_ext_property_id(pst, prop);
		if (prop_id < 0)
			return prop_id;
...
EXPORT_SYMBOL(qti_battery_charger_get_ext_prop);
...
EXPORT_SYMBOL(qti_power_supply_changed);
...
const char *qti_battery_charger_get_real_type(const char *name)
{
	...
	return get_usb_type_name(pst->prop[USB_REAL_TYPE]);
}
EXPORT_SYMBOL(qti_battery_charger_get_real_type);
				...
				enum lenovo_supply_property prop, int val)
{
	...
	case LENOVO_SUPPLY_PROP_TYPEC_CONNECT_ST:
		...
		if (!rc)
			pr_info("set adsp usb property successful,val is %d\n", val);
		break;
	default:
		pst = &bcdev->psy_list[PSY_TYPE_BATTERY];
		...
		rc = write_property_id(bcdev, pst, prop_id, val);
		if (!rc)
			pr_info("set adsp property successful,val is %d\n", val);
		break;
	}

	return rc;
}
EXPORT_SYMBOL(qti_battery_charger_set_ext_prop);
#endif

static int get_user_battery_prop(struct battery_chg_dev *bcdev,
		enum lenovo_supply_property psp,
		union lenovo_supply_propval *pval)
{
	int rc = -EINVAL;

	if (!bcdev->fg_user_psy) {
		bcdev->fg_user_psy = lenovo_supply_get_by_name("bq-battery");
		if (!bcdev->fg_user_psy) {
			pr_err("Couldn't find user prop\n");
			return rc;
		}
	}

	rc = lenovo_supply_get_property(bcdev->fg_user_psy, psp, pval);
	if (rc < 0) {
		pr_err("Couldn't get user prop, rc=%d, psp=%d\n", rc, psp);
		return rc;
	}
...
#if 0
static int get_qcom_usb_bak_prop(struct battery_chg_dev *bcdev,
		enum lenovo_supply_property psp,
		union lenovo_supply_propval *pval)
{
	int rc = -EINVAL;

	if (!bcdev->qcom_usb_bak_psy) {
		bcdev->qcom_usb_bak_psy = lenovo_supply_get_by_name("qcom-usb");
		if (!bcdev->qcom_usb_bak_psy) {
			pr_err("Couldn't find qcom usb bak prop\n");
			return rc;
		}
	}

	rc = lenovo_supply_get_property(bcdev->qcom_usb_bak_psy, psp, pval);
	if (rc < 0) {
		pr_err("Couldn't get qcom usb bak prop, rc=%d, psp=%d\n", rc, psp);
		return rc;
	}
...
static int get_bq2589x_prop(struct battery_chg_dev *bcdev,
		enum lenovo_supply_property psp,
		union lenovo_supply_propval *pval)
{
	int rc = -EINVAL;

	if (!bcdev->bq2589x_psy) {
		bcdev->bq2589x_psy = lenovo_supply_get_by_name("bq2589h-charger");
		if (!bcdev->bq2589x_psy) {
			pr_err("Couldn't find bq2589x prop\n");
			return rc;
		}
	}

	rc = lenovo_supply_get_property(bcdev->bq2589x_psy, psp, pval);
	if (rc < 0) {
		pr_err("Couldn't get bq2589x prop, rc=%d, psp=%d\n", rc, psp);
		return rc;
	}
...
static int get_user_usb_online(struct battery_chg_dev *bcdev)
{
	union lenovo_supply_propval val = {0, };
	int on = 0;

	if (get_bq2589x_prop(bcdev, LENOVO_SUPPLY_PROP_HLOS_CHG_ONLINE, &val) < 0)
		on = 0;
	else
		on = val.intval;

	if (get_bq2589x_prop(bcdev, LENOVO_SUPPLY_PROP_TYPEC_CC_ORIENTATION, &val) < 0)
		on |= 0;
	else
		on |= val.intval;
		...
		union power_supply_propval *pval)
{
	...
	if (prop == POWER_SUPPLY_PROP_ONLINE) {
		pval->intval = get_user_usb_online(bcdev);
		return 0;
	}
#endif
	prop_id = get_property_id(pst, prop);
...
#ifdef CONFIG_LENOVO_SUPPLY
int g_charge_thermal_level = 0;
EXPORT_SYMBOL(g_charge_thermal_level);
int g_charge_thermal_fcc = 0;
EXPORT_SYMBOL(g_charge_thermal_fcc);
#endif
					...
					int val)
{
	...
	g_charge_thermal_level = bcdev->curr_thermal_level;
	g_charge_thermal_fcc = bcdev->thermal_fcc_ua;
	rc = 0;
#else
...
#else
	case POWER_SUPPLY_PROP_CAPACITY:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_CAPACITY, &val) < 0)
			pval->intval = DEFAULT_USER_CAPACITY;
		else
			pval->intval = val.intval;
		break;
	case POWER_SUPPLY_PROP_TEMP:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_TEMP, &val) < 0)
			pval->intval = DEFAULT_USER_TEMP;
		else
			pval->intval = val.intval;
		break;
	case POWER_SUPPLY_PROP_STATUS:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_STATUS, &val) < 0)
			pval->intval = DEFAULT_USER_STATUS;
		else
			pval->intval = val.intval;
		break;
	case POWER_SUPPLY_PROP_HEALTH:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_HEALTH, &val) < 0)
			pval->intval = DEFAULT_USER_HEALTH;
		else
			pval->intval = val.intval;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_VOLTAGE_NOW, &val) < 0)
			pval->intval = DEFAULT_USER_VOLTAGE_NOW;
		else
			pval->intval = val.intval;
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_CURRENT_NOW, &val) < 0)
			pval->intval = DEFAULT_USER_CURRENT_NOW;
		else
			pval->intval = val.intval;
		break;
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_CHARGE_COUNTER, &val) < 0)
			pval->intval = DEFAULT_USER_CHARGE_COUNTER;
		else
			pval->intval = val.intval;
		break;
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_CYCLE_COUNT, &val) < 0)
			pval->intval = DEFAULT_USER_CYCLE_COUNT;
		else
			pval->intval = val.intval;
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_CHARGE_FULL_DESIGN, &val) < 0)
			pval->intval = DEFAULT_USER_CHARGE_FULL_DESIGN;
		else
			pval->intval = val.intval;
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		if (get_user_battery_prop(bcdev, LENOVO_SUPPLY_PROP_CHARGE_FULL, &val) < 0)
			pval->intval = DEFAULT_USER_CHARGE_FULL;
		else
			pval->intval = val.intval;
		break;
#endif
...
#ifdef CONFIG_LENOVO_SUPPLY
	for (i = 0; i < len; i++) {
		pr_info("batt_sys: thermal level %d fcc %d\n", i, bcdev->thermal_levels[i]);
	}
#endif
