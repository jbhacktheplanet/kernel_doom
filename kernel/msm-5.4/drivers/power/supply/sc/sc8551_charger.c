...
#define SC8551_ROLE_MASTER	1
#define SC8551_ROLE_SLAVE	2
...
struct sc8551 {
	...
	struct lenovo_supply_desc psy_desc;
	struct lenovo_supply_config psy_cfg;
	struct lenovo_supply *fc2_psy;
};
...
static enum lenovo_supply_property sc8551_charger_props[] = {
...
static int sc8551_charger_get_property(struct lenovo_supply *psy,
				enum lenovo_supply_property psp,
				union lenovo_supply_propval *val)
{
	struct sc8551 *sc = lenovo_supply_get_drvdata(psy);
	int result;
	int ret;
	u8 reg_val;
	result = 0;
	...
	case LENOVO_SUPPLY_PROP_SC_BATTERY_VOLTAGE:
		...
		break;
	case LENOVO_SUPPLY_PROP_SC_BATTERY_CURRENT:
		...
		break;
	case LENOVO_SUPPLY_PROP_SC_BATTERY_TEMPERATURE:
		...
		break;
	case LENOVO_SUPPLY_PROP_SC_BUS_VOLTAGE:
		...
		break;
	case LENOVO_SUPPLY_PROP_SC_BUS_CURRENT:
		...
		break;
	case LENOVO_SUPPLY_PROP_SC_BUS_TEMPERATURE:
		...
		break;
	case LENOVO_SUPPLY_PROP_SC_DIE_TEMPERATURE:
		...
		break;
	case LENOVO_SUPPLY_PROP_SC_ALARM_STATUS:
	...
	case LENOVO_SUPPLY_PROP_SC_FAULT_STATUS:
	...
	case LENOVO_SUPPLY_PROP_SC_VBUS_ERROR_STATUS:
...
static int sc8551_charger_set_property(struct lenovo_supply *psy,
				       ...
				       const union lenovo_supply_propval *val)
{
	...
	case LENOVO_SUPPLY_PROP_CHARGING_ENABLED:
		...
		sc8551_check_charge_enabled(sc, &sc->charge_enabled);
		sc_info("LENOVO_SUPPLY_PROP_CHARGING_ENABLED: %s\n",
...
static int sc8551_charger_is_writeable(struct lenovo_supply *psy,
				       enum lenovo_supply_property prop)
{
	int ret;
...
static int sc8551_psy_register(struct sc8551 *sc)
{
	...
	sc->psy_desc.type = LENOVO_SUPPLY_TYPE_SC;
	...
	sc->fc2_psy = devm_lenovo_supply_register(sc->dev, 
	...
	sc_info("%s lenovo supply register successfully\n", sc->psy_desc.name);
 ...
 * through lenovo supply interface
 */
static irqreturn_t sc8551_charger_interrupt(int irq, void *dev_id)
{
	...
	mutex_unlock(&sc->irq_complete);
#endif
	lenovo_supply_changed(sc->fc2_psy);
...
err_1:
	lenovo_supply_unregister(sc->fc2_psy);
	return ret;
}
...
static void sc8551_charger_shutdown(struct i2c_client *client)
{
	struct sc8551 *sc = i2c_get_clientdata(client);
	sc8551_enable_adc(sc, false);
	pr_info("port 2 sc8551 shutdown ...");
}
