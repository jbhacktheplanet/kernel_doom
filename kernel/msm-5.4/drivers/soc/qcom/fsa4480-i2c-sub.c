				      ...
				      unsigned long evt, void *ptr)
{
	...
	struct device *dev;
	enum typec_accessory acc = audio_switch_state;
...
static int fsa4480_usbc_analog_setup_switches(struct fsa4480_priv *fsa_priv)
{
	...
	/* get latest mode again within locked context */
       mode = audio_switch_state;
	pr_info("%s: %d mode.intval=%d",__func__,__LINE__,mode);
	switch (mode) {
	...
	case 1:
		...
		if(fsa4480_logic_data->irq_gpio){
			...
			pr_info("%s: %d gpio_direction_output=%d",__func__,__LINE__,mode);
			msleep(50);
		}
			 ...
			 const struct i2c_device_id *id)
{
	...
	atomic_set(&(fsa_priv->usbc_mode), 0xff);
...
/*	power_supply_put(fsa_priv->psy_nb);*/
...
MODULE_DESCRIPTION("FSA4480 I2C driver doom");
